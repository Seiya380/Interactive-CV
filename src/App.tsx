...
const BotAvatar = () => (
  <div className="flex-shrink-0 flex items-center justify-center w-10 h-10 overflow-hidden p-[2px]" style={{border: '1px solid var(--cp-border)', boxShadow: 'var(--cp-glow)'}}>
...
export default function App() {
  ...
  return (
    <div className="flex flex-col h-screen print:h-auto print:bg-white cp-root">
      {/* En-tête (Header) */}
      <header id="print-header" className="cp-header py-3 px-6 flex items-center justify-between z-10 print:px-0 print:py-6">
        <div className="flex items-center">
          <div className="mr-4">
            <img
              src={profilePhoto}
              alt={identity.basics.name}
              className="w-16 h-16 object-cover cp-photo cursor-pointer hover:scale-[1.3] transition-all duration-300"
              onClick={() => setIsPhotoModalOpen(true)}
            />
          </div>
          <div>
            <h1 className="cp-title text-lg">CV interactif de {identity.basics.name}</h1>
            <div className="flex flex-wrap items-center gap-4 mt-1 text-sm">
              <a
                href={`mailto:${identity.basics.email}`}
                className="cp-link flex items-center gap-1.5"
              >
        ...
        <div className="flex items-center gap-2">
          <span className="cp-version text-xs font-mono py-1 px-2 print:hidden">
            {APP_VERSION}
          </span>
          <a
            href={cvPdfLink}
            onClick={handleDownload}
            className="cp-icon-btn cursor-pointer print:hidden"
      ...
      <main id="print-chat" className="cp-main flex-1 overflow-y-auto p-4 sm:p-6 w-full max-w-4xl mx-auto print:overflow-visible print:p-0 print:block">
        <div className="space-y-6 print:space-y-4">
          {messages.map((msg) => (
            ...
            >
              {/* Avatar */}
              {msg.role === 'user' ? (
                <div className="cp-avatar-user flex-shrink-0 w-10 h-10 flex items-center justify-center">
              ...
              {/* Bulle de message */}
              <div className={`max-w-[85%] sm:max-w-[75%] px-5 py-4 ${msg.role === 'user'
                  ? 'cp-bubble-user'
                  : 'cp-bubble-bot'
                }`}>
                {msg.role === 'user' ? (
          ...
          {isLoading && (
            <div className="flex gap-4 flex-row">
               <BotAvatar />
              <div className="cp-loader-bubble px-5 py-4 flex items-center gap-1">
                <Loader2 className="w-4 h-4 animate-spin mr-2" style={{color: 'var(--cp-border)'}} />
                <div className="dot-loading text-xl flex gap-1">
      ...
      <footer className="cp-footer p-4 sm:p-6 print:hidden">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-2">
            <div className="flex gap-2">
              <input
                ...
                disabled={isLoading}
                maxLength={500}
                className="cp-input flex-1 text-sm block w-full p-3 sm:p-4 disabled:opacity-50"
              />
              <button
                type="submit"
                disabled={!input.trim() || isLoading}
                className="cp-send-btn px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-center"
              >
            ...
            <div className="flex justify-end pr-1">
              <span className={`cp-char-count font-medium transition-colors ${input.length > 450 ? 'warn' : ''}`}>
                {input.length} / 500
              </span>
            </div>
          </form>
